...
const initialCartTotalQuantity = cartItemsFromLocalStorage.reduce((total, item) => {
    return total + item.cartQuantity;
}, 0);

const initialState = {
  cartItems : localStorage.getItem("cartItems") ? JSON.parse(localStorage.getItem("cartItems")) : [],
  cartTotalQuantity : initialCartTotalQuantity,
  cartTotalAmount: 0,
  previousURL : ""
};
...
const cartSlice = createSlice({
    ...
    reducers: {
        ADD_TO_CART: (state, action) => {
            ...
            if(productIndex >= 0){
                // item already exist in the cart, increase the quantity of item
                state.cartItems[productIndex].cartQuantity += 1
                toast.info(`${action.payload.name} increased by one`, { position: "top-left"})
            }else{
                // item doesn't exist in the cart, add item to the cart
...
export const { ADD_TO_CART, DECREASE_CART_QTY, REMOVE_FROM_CART, CLEAR_CART, CALCULATE_SUBTOTAL, CALCULATE_TOTAL_QUANTITY, SAVE_URL } = cartSlice.actions;
